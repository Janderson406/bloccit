require 'rails_helper'
 ...
 #we want unauthenticated users to be able to fetch a topic or all topics, per our access rules.
  ...
  #unauthenticated users cannot EDIT/CREATE/DELETE TOPICS
 ...
 #we want unauthorized users to be able to fetch a topic or all topics, per our access rules
  ...
  #unAUTHorized users cannot EDIT/CREATE/DELETE TOPICS
   ...
   context "authenticated and authorized users" do
     #we make my_user an admin and set the header to authenticate them. We then use FactoryGirl to build @new_topic
     before do
       my_user.admin!
       controller.request.env['HTTP_AUTHORIZATION'] = ActionController::HttpAuthentication::Token.encode_credentials(my_user.auth_token)
  ...
  #API UPDATE TOPIC
     describe "PUT update" do
       ...
       #we test that my_topic was properly updated
  ...
  #API DELETE TOPIC
     describe "DELETE destroy" do
       ...
       it "deletes my_topic" do
         # we expect that searching for the deleted topic will raise an ActiveRecord::RecordNotFound exception.
