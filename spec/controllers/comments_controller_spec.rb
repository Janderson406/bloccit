...
include SessionsHelper
     ...
     describe "POST create" do
       it "redirects the user to the sign in view" do
         post :create, format: :js, post_id: my_post.id, comment: {body: RandomData.random_paragraph}
          # we pass format: :js to the delete method. This simulates a JavaScript request to the controller.
 ...
 #allow member users to create new comments, redirect them to posts show view if they try to delete another user's comment.
 ...
 #we test that members users are able to create new comments and delete their own comments.
   context "member user doing CRUD on a comment they own" do
     ...
     describe "DELETE destroy" do
       ...
       #we no longer expect that the destroy action will redirect to the post show view. Instead we expect to receive an HTTP success message.
       it "returns http success" do
         delete :destroy, format: :js, post_id: my_post.id, id: my_comment.id
         expect(response).to have_http_status(:success)
       end
